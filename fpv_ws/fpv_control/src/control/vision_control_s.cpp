...
#include <fpv_control/msg/image_deviation.hpp>
#include <stdint.h>
#include <mutex>
#include <cmath>
#include <chrono>
...
{
public:
    VisionControl() : Node("sh_vision_control_s"),
                      ...
                      target_velocity_z_(0.0)
    {
        ...
        // 创建订阅器
        image_deviation_sub_ = this->create_subscription<fpv_control::msg::ImageDeviation>(
            deviation_topic_, rclcpp::SensorDataQoS(),
            [this](const fpv_control::msg::ImageDeviation::SharedPtr msg) {
    ...
    ~VisionControl() {
        if (data_file_.is_open()) {
            data_file_.close();
            RCLCPP_INFO(this->get_logger(), "Data txt file closed: %s", full_log_path_.c_str());
        }
    }
...
private:
    ...
    rclcpp::Subscription<fpv_control::msg::ImageDeviation>::SharedPtr image_deviation_sub_;
    ...
    std::string log_directory_ = "/home/sh/sh_ws/document/datelogs/";
    std::string base_log_filename_ = "uavdate_";  // 文件名前缀
    std::string log_time_suffix_;                 // 文件名时间后缀
    std::string full_log_path_;                   // 完整文件路径
    
    /**
     * 生成当前时间的日志文件名后缀，格式：年月日_时分秒（如20251112_153045）
     */
    void generate_time_based_filename() {
        auto now = std::chrono::system_clock::now();
        std::time_t now_time = std::chrono::system_clock::to_time_t(now);
        std::tm* local_time = std::localtime(&now_time);

        std::stringstream time_ss;
        time_ss << std::put_time(local_time, "%Y%m%d_%H%M%S");
        log_time_suffix_ = time_ss.str();

        full_log_path_ = log_directory_ + base_log_filename_ + log_time_suffix_ + ".txt";
        RCLCPP_INFO(this->get_logger(), "Generated log file path: %s", full_log_path_.c_str());
    }
    ...
    void init_data_log() {
        ...
        if (stat(log_directory_.c_str(), &info) != 0) {
            if (mkdir(log_directory_.c_str(), 0777) == -1) {
                RCLCPP_ERROR(this->get_logger(), "Failed to create log directory: %s", log_directory_.c_str());
                return;
            } else {
                RCLCPP_INFO(this->get_logger(), "Created log directory: %s", log_directory_.c_str());
            }
        }

        data_file_.open(full_log_path_, std::ios::out | std::ios::app);
        if (data_file_.is_open()) {
            RCLCPP_INFO(this->get_logger(), "Data logging to: %s", full_log_path_.c_str());
            if (data_file_.tellp() == 0) {
                data_file_ << "system_time position_x position_y position_z "
                           << "attitude_q0 attitude_q1 attitude_q2 attitude_q3 yaw "
                           << "angle_x angle_y" << std::endl;
            }
        } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to open log file: %s", full_log_path_.c_str());
        }
    }
    ...
    void log_data() {
        ...
        {
            std::lock_guard<std::mutex> lock(odometry_mutex_);
            pos_x = current_position_x_;
            pos_y = current_position_y_;
            pos_z = current_position_z_;
        }

        std::array<float, 4> attitude_q;
        double yaw;
        {
            std::lock_guard<std::mutex> lock(attitude_mutex_);
            attitude_q = current_attitude_q_;
            yaw = current_yaw_;
        }
        ...
        data_file_ << std::put_time(local_time, "%Y-%m-%d %H:%M:%S") << " "
                   << std::fixed << std::setprecision(6)
                   << pos_x << " " << pos_y << " " << pos_z << " "
                   << attitude_q[0] << " " << attitude_q[1] << " " << attitude_q[2] << " " << attitude_q[3] << " "
                   << yaw << " "
                   << dev_x << " " << dev_y << std::endl;
    }
