...
class E2EStateMachine(Node):
    def __init__(self):
        ...
        model_path = os.path.join(home_dir, 'EndToEnd_FPV/E2E_FPV/E2E_FPV_ResNet18/models/resnet_e2e_model_v1.pth')
        stats_path = os.path.join(home_dir, 'EndToEnd_FPV/E2E_FPV/E2E_FPV_ResNet18/models/label_stats_v1.json')
        ...
        try:
            with open(stats_path, 'r') as f:
                stats = json.load(f)
            self.label_mean = np.array(stats['mean'], dtype=np.float32)
            self.label_std = np.array(stats['std'], dtype=np.float32)
            self.get_logger().info(f"Loaded label stats: Mean={self.label_mean}, Std={self.label_std}")
        except Exception as e:
            self.get_logger().error(f"Failed to load label stats JSON: {e}")
            self.label_mean = np.zeros(4, dtype=np.float32)
            self.label_std = np.ones(4, dtype=np.float32)

        # 加载 ResNet 模型
        self.model = ResNetPilotNet(output_dim=4).to(self.device)
        try:
            self.model.load_state_dict(torch.load(model_path, map_location=self.device))
            self.model.eval()
            self.get_logger().info(f"ResNet Model loaded: {model_path}")
        except Exception as e:
            self.get_logger().error(f"Failed to load ResNet model: {e}")
        ...
        self.get_logger().info("ResNet E2E Control Started")
    ...
    def img_callback(self, msg):
        ...
        try:
            ...
            # --- 反归一化 ---
            prediction = (pred_norm * self.label_std) + self.label_mean
    ...
    def timer_callback(self):
        ...
        if self.offboard_setpoint_counter == 150 and not self.is_armed_recorded:
            ...
            self.base_altitude = self.current_pos[2] 
        ...
        if self.current_state == State.TAKEOFF:
            ...
            if self.offboard_setpoint_counter % 20 == 0:
                self.get_logger().info(f"Vel_NED: {vel_ned} | Yaw_Rate: {self.net_yaw_rate:.2f}")
